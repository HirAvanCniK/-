    ...
    try {
      ...
      for (let i = 0; i < queue.length; i += 10) {
        ...
        const embed = new Discord.MessageEmbed()
          ...
          .setDescription(
            `**Current Song - [\`${queue[0].name}\`](${queue[0].url})**\n\n${info}`
          )
  ...
  playsongyes: async function (client, message, queue, song) {
    try {
      ...
      let embed1 = new Discord.MessageEmbed()
        ...
        .addField(
          "↪️ Autoplay:",
          `>>> ${queue.autoplay ? ":white_check_mark:" : ":x:"}`,
          true
        )
      ...
      const filter = (reaction, user) =>
        [
          ...
          "805375191462707230",
          "805375191307649055",
        ].includes(reaction.emoji.id) && user.id !== message.client.user.id;
      ...
      collector.on("collect", async (reaction, user) => {
        ...
        let djrolefromenmap = reaction.message.guild.roles.cache.get(
          client.settings.get(reaction.message.guild.id, `djrole`)
        );
        if (
          !member.hasPermission("ADMINISTRATOR") &&
          djrolefromenmap &&
          !member.roles.cache.has(djrolefromenmap.id)
        )
          return embedbuilder(
            client,
            5000,
            ...
            config.colors.no,
            "DJ-ROLE",
            `<:NO:805375191819616266> You don\'t have permission for this Command! You need to have: ${djrolefromenmap}`
          );
        ...
        switch (reaction.emoji.id) {
          ...
          case "805375191487873024":
            ...
            functions.embedbuilder(
              ...
              config.colors.yes,
              "Volume!",
              `Reduced the Volume to \`${queue.volume}\``
            );
            break;
        ...
        const embed = new Discord.MessageEmbed()
          ...
          .setFooter(
            client.user.username +
              ` ・ Current Page - ${i + 1} / ${client.categories.length}`,
            config.AVATARURL
          );
        embeds.push(embed);
      }

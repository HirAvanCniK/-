...
module.exports = {
  ...
  usage: "mycustom <add/remove/play/reset> [link]",
  ...
  run: async (client, message, args) => {
    ...
    if (args[0] === "reset" || args[0] === "res") {
      let themsg = await message.reply(
        "Do you really want to reset your custom playlist? ||(*Reply with:* **__`yes`__**)||"
      );
    ...
    if (
      ...
      return functions.embedbuilder(
        ...
        message,
        config.colors.yes,
        "SUCCESSFULLY REMOVED THE SONG FROM YOUR PLAYLIST"
      );
    } else {
      ...
      functions.embedbuilder(
        ...
        message,
        config.colors.yes,
        `There are ${playlist.length} Songs in your Playlist!`
      );
      return functions.embedbuilder(
        client,
        "null",

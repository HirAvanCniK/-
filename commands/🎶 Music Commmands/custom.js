...
module.exports = {
  ...
  usage: "custom <add/remove/play/reset> [link]",
  ...
  run: async (client, message, args) => {
    ...
    if (args[0] === "add" || args[0] === "set" || args[0] === "use") {
      if (!args[1].includes("http"))
        return message.reply(
          "Oh no! That is not a Link for example: https://www.youtube.com/watch?v=dQw4w9WgXcQ"
        );
      if (playlist.includes(args[1]))
        return message.reply(
          "Oh no! The Song is already existing in the Server Playlist, NO DOUBLE SONGS!"
        );
      ...
      return functions.embedbuilder(
        ...
        message,
        config.colors.yes,
        "SUCCESSFULLY ADDED A SONG TO THE SERVER PLAYLIST" +
          `
    ...
    if (args[0] === "reset" || args[0] === "res") {
      let themsg = await message.reply(
        "Do you really want to reset your the Server playlist? ||(*Reply with:* **__`yes`__**)||"
      );
      ...
      themsg.channel
        ...
        .then(async (collected) => {
          if (collected === "yes") {
            ...
            await message.reply(
              "SUCCESSFULLY RESETTED THE SERVER CUSTOM PLAYLIST"
            );
          }
        })
        .catch((error) => {
    ...
    if (
      ...
      if (!args[1])
        return message.reply(
          "Please add a song link what you want to add, thanks!"
        );
      if (!playlist.includes(args[1]))
        return message.reply(
          "Oh no! The Song is not existing, in the Server playlist, make sure it is the same link!"
        );
      ...
      return functions.embedbuilder(
        ...
        message,
        config.colors.yes,
        "SUCCESSFULLY REMOVED THE SONG FROM THE SERVER PLAYLIST"
      );
    } else {
      ...
      functions.embedbuilder(
        ...
        message,
        config.colors.yes,
        `There are ${playlist.length} Songs in the Server Playlist!`
      );
      return functions.embedbuilder(
        client,
        "null",
...
async function customplay(message, string, cursong) {
  let currentPage = 0;
  const embeds = functions.customplaylistembed(
    client,
    message,
    string,
    cursong

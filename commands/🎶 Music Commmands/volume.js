...
module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    if(args[0] > 100) return message.channel.send("Il numero inserito è troppo alto!");
    if(args[0] < 0) return message.channel.send("Il numero inserito è troppo basso!");
    ...
    if (Number(args[0]) > 100 && Number(args[0]) < 0) return functions.embedbuilder(client, "null", message, config.colors.no, "Not valid Number", "Please use a volume Number between `0` and `500`")

    ...
    if (!args[0])
      return message.channel.send(
        new Discord.MessageEmbed()
          ...
          .setTitle(":x: Error | Unknown Command Usage!")
          .setDescription(
    ...
    translate(args.slice(2).join(" "), { from: args[0], to: args[1] })
      .then((res) => {
        let embed = new Discord.MessageEmbed()
          ...
          .setFooter("IRVANNI | powered by: IRVANNI", config.AVATARURL)
          .addField(
            `From: \`${args[0]}\``.substr(0, 256),
            args.slice(2).join(" ").substr(0, 1024)
          )
          .addField("\u200B", "\u200B")
          .addField(
            `To: \`${args[1]}\``.substr(0, 256),
            res.text.substr(0, 1024)
          );
      ...
      .catch((err) => {
        let embed = new Discord.MessageEmbed()
          ...
          .setFooter("IRVANNI", config.AVATARURL)

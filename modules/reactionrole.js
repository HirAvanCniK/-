...
module.exports = (client) => {
  ...
  client.on("messageReactionAdd", async (reaction, user) => {
    try {
      ...
      const reactionsetup = client.reactionrole.get(
        reaction.message.guild.id,
        "reactionroles"
      );
      for (let k = 0; k < reactionsetup.length; k++) {
        if (reaction.message.id === reactionsetup[k].MESSAGE_ID) {
          let messagereaction = reaction.message.guild.members.cache.get(
            user.id
          );
            ...
            try {
              for (const reaction of userReactions.values()) {
                if (
                  ...
                  oldreact.emoji.name != reaction.emoji.name
                ) {
      ...
      console.log(
        "REACTIONROLE:".underline.red + " :: " + e.stack.toString().red
      );
    }
  });

...
module.exports = function (client, options) {
  ...
  let disabled = new MessageEmbed()
    ...
    .setFooter("The Supreme Bot", config.AVATARURL)
  ...
  client.on("messageReactionAdd", async (reaction, user) => {
    ...
    if (
      message.channel.id === client.apply.get(message.guild.id, "channel_id") &&
      reaction.emoji.name === "✅"
    ) {
      reaction.users.remove(user);
      let guild = await message.guild.fetch();
      let channel_tosend = guild.channels.cache.get(
        client.apply.get(message.guild.id, "f_channel_id")
      );
            ...
            msg.channel
              ...
              .catch((error) => {
                ...
                return message.channel
                  .send(`${user}, SORRY BUT YOUR TIME RAN OUT!`)
                  .then((msg) => msg.delete({ timeout: 3000 }));
              });
          })
          .catch((e) => {
            reaction.message.channel
              .send(`${user}, SORRY! But i cant dm you :'(`)
              .then((msg) => msg.delete({ timeout: 3000 }));

            console.log(
              "APPLY:".underline.red + " :: " + e.stack.toString().red
            );
          });
      }
        ...
        for (let i = 0; i < Questions.length; i++) {
          try {
            embed.addField(
              ...
              String(answers[i]).substr(0, 1024)
            );
          } catch {}
        }
        channel_tosend
        ...
        user
          .send(
            new Discord.MessageEmbed()
              ...
              .setDescription(`${reaction.message.channel}`)
              .setFooter(
                message.guild.name,
                message.guild.iconURL({ dynamic: true })
              )
    ...
    if (
      ...
      if (reaction.emoji.name === "✅") {
        ...
        let approve = new Discord.MessageEmbed()
          .setFooter(
            message.guild.name,
            message.guild.iconURL({ dynamic: true })
          )
        ...
        let usert = await client.users.fetch(
          client.apply.get(message.id, "temp")
        );
      ...
      if (reaction.emoji.name === "❌") {
        embed.setColor("RED");
        targetMessage.edit(embed);
        let deny = new Discord.MessageEmbed()
          .setFooter(
            message.guild.name,
            message.guild.iconURL({ dynamic: true })
          )

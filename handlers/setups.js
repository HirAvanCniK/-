...
module.exports = (client) => {
  ...
  client.on("ready", () => {
    setInterval(() => {
      ...
      if (stateswitch)
        client.user.setActivity(`${config.prefix}help | IRVANNI`, {
          type: "PLAYING",
        });
      else
        client.user.setActivity(
          `${client.guilds.cache.reduce(
            (c, g) => c + g.memberCount,
            0
          )} User | ${client.guilds.cache.size} Server`,
          { type: "PLAYING" }
        );
    }, 5000); //5 second delay
  });
    ...
    client.setups.set(guild.id, {
      ...
      welcome: {
        ...
        background: {
          ...
          image:
            "https://images.hdqwalls.com/download/gfx-nerds-uz-2560x1440.jpg",
        },
      },
      leave: {
        enabled: false,
      ...
      ticketsystem: {
        ...
        parentid: "",
        message:
          "Hey {user}, thanks for opening an ticket! Someone will help you soon!",
        adminroles: [],
      },
      jointocreate: {
        enabled: false,
  ...
  client.on("channelDelete", function (channel) {
    ...
    try {
      client.setups.remove(channel.guild.id, channel.id, `botchannel`);
    } catch {}
    try {
      if (client.setups.get(channel.guild.id, `channel_id`) === channel.id)
        client.setups.set(channel.guild.id, "", `channel_id`);
    } catch {}
    try {
      if (client.setups.get(channel.guild.id, `f_channel_id`) === channel.id)
        client.setups.set(channel.guild.id, "", `f_channel_id`);
    } catch {}
    try {
      if (client.setups.get(channel.guild.id, `logger.channel`) === channel.id)
        client.setups.set(channel.guild.id, "", `logger.channel`);
    } catch {}
    try {
      if (client.setups.get(channel.guild.id, `welcome.channel`) === channel.id)
        client.setups.set(channel.guild.id, "", `welcome.channel`);
    } catch {}
    try {
      if (client.setups.get(channel.guild.id, `leave.channel`) === channel.id)
        client.setups.set(channel.guild.id, "", `leave.channel`);
    } catch {}
    try {
      if (
        client.setups.get(channel.guild.id, `membercount.channel`) ===
        channel.id
      )
        client.setups.set(channel.guild.id, "", `membercount.channel`);
    } catch {}
    try {
      if (
        client.setups.get(channel.guild.id, `aichatsystem.channel`) ===
        channel.id
      )
        client.setups.set(channel.guild.id, "", `aichatsystem.channel`);
    } catch {}
    try {
      if (
        client.setups.get(channel.guild.id, `ticketsystem.parentid`) ===
        channel.id
      )
        client.setups.set(channel.guild.id, "", `ticketsystem.parentid`);
    } catch {}
    try {
      if (
        client.setups.get(channel.guild.id, `ticketsystem.channelid`) ===
        channel.id
      )
        client.setups.set(channel.guild.id, "", `ticketsystem.channelid`);
    } catch {}
    try {
      if (client.setups.get(channel.guild.id, `counter.channel`) === channel.id)
        client.setups.set(channel.guild.id, "", `counter.channel`);
    } catch {}
    try {
  ...
  client.on("roleDelete", function (role) {
    ...
    try {
      if (client.apply.get(role.guild.id, `TEMP_ROLE`) === role.id)
        client.setups.set(channel.guild.id, "", `TEMP_ROLE`);
    } catch {}
    try {
      client.settings.remove(role.guild.id, role.id, `welcome.roles`);
    } catch {}
    try {
      client.setups.remove(
        channel.guild.id,
        channel.id,
        `ticketsystem.adminroles`
      );
    } catch {}
  });

...
client.on("ready", () => {
  var data = new Date();
  console.log(
    `Running at ${data.getHours()}:${data.getMinutes()} ${data.getDate()}/${data.getMonth() + 1
    }/${data.getFullYear()}`
  );
  console.log(`${client.user.username} is now online`); // log to console
...
client.on("voiceStateUpdate", async (oldState, newState) => {
  try {
    if (
      newState.channel.id === config.voicechannel &&
      newState.guild.id === config.guildid
    ) {
...
async function radioexecuteadmin() {
  ...
  const voiceChannel = client.guilds.cache
    .get(config.guildid)
    .channels.cache.get(config.voicechannel);
  //join the channel
  voiceChannel.join().then((cnc) => {
    cnc.voice.setSelfDeaf(true); //set self deaf
    cnc.voice.setDeaf(true); //set server self deaf
    cnc
      .play("http://mp3stream7.apasf.apa.at:8000/.mp3") //play the stream
